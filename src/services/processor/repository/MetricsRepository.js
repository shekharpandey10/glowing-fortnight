...
export class MetricsRepository extends BaseRepository {
    constructor({ logger: l, postgres: pg } = {}) {
        super({ logger: l })
        if (!pg) throw new Error('Pg requires for MetricsRepository')
        this.postgres = pg
    }
    ...
    async upsertEndpointMetrics(metricsData) {
        try {
            ...
            client_id,service_name,endpoint,method,total_hits,error_hits,avg_latency,min_latency,max_latency,time_bucket
            )
            ...
            this.logger.error("Error upserting endpoint_metrics ", error)
            throw error
        }
    }
    ...
    async getMetrics(filter = {}) {
        try {
            ...
            let query = `
                SELECT 
                    ...
                    SUM(min_latency) as min_latency ,
        ...
        OFFSET $${paramIndex + 1}
           `
            ...
            this.logger.error("Error while getting endpoint_metrics ", error)
            throw error
        }
    }
    _query(sql, params = [], client = this.postgres) {
        ...
        if (!target || typeof target.query !== 'function') {
            const error = new Error("Postgres client not configured on MetricsRepository.")
